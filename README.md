...
const nysaFeedData = [
  ...
  },
  {
    ...
    primarySupport: {
      ...
      email: "appointments@drsharmadental.com"
    },
    details: {
...
const nysaFeedForms = {
  "Service Providers": (feed) => `
    <div class="nysa-form-container">
      <h3>Book ${feed.details.title}</h3>
      <form id="serviceProviderForm">
        ...
        <div class="form-group">
          ...
          <textarea id="service-address" rows="3" required></textarea>
        </div>
  ...
  "Healthcare": (feed) => `
    <div class="nysa-form-container">
      <h3>Book Appointment with ${feed.details.provider}</h3>
      <form id="healthcareForm">
        ...
        <div class="form-group">
          ...
          <input type="number" id="patient-age" min="1" required>
        </div>
  ...
  "Jobs": (feed) => `
    <div class="nysa-form-container">
      <h3>Apply for ${feed.details.title}</h3>
      <form id="jobApplicationForm">
        ...
        <div class="form-group">
          <label for="applicant-resume">Resume (URL or brief summary)</label>
          <textarea id="applicant-resume" rows="3" required></textarea>
        </div>
        ...
        <div class="form-group">
          <label for="applicant-skills">Key Skills</label>
          <textarea id="applicant-skills" rows="2" required></textarea>
        </div>
        
        <div class="form-group">
          <label for="applicant-notice">Notice Period (days)</label>
          <input type="number" id="applicant-notice" min="0" required>
        </div>
        
        <div class="form-group">
          <label for="applicant-cover">Cover Letter (optional)</label>
          <textarea id="applicant-cover" rows="4"></textarea>
        </div>
  ...
  // Add forms for other categories similarly
  "Real Estate": (feed) => `
    <div class="nysa-form-container">
      <h3>Schedule Visit for ${feed.details.title}</h3>
      <form id="realEstateForm">
        ...
        <div class="form-group">
          <label for="visit-members">Number of People Visiting</label>
          <input type="number" id="visit-members" min="1" value="1" required>
        </div>
        ...
        <button type="submit" class="submit-form-btn">
          <i class="fab fa-whatsapp"></i> Schedule via WhatsApp
        </button>
      </form>
    </div>
  `,
  
  "Government Schemes": (feed) => `
    <div class="nysa-form-container">
      <h3>Apply for ${feed.details.title}</h3>
      <form id="governmentSchemeForm">
        <div class="form-group">
          <label for="scheme-name">Full Name</label>
          <input type="text" id="scheme-name" required>
        </div>
        
        <div class="form-group">
          <label for="scheme-dob">Date of Birth</label>
          <input type="date" id="scheme-dob" required>
        </div>
        
        <div class="form-group">
          <label for="scheme-gender">Gender</label>
          <select id="scheme-gender" required>
            <option value="">Select gender</option>
            <option value="male">Male</option>
            <option value="female">Female</option>
            <option value="other">Other</option>
        ...
        <div class="form-group">
          <label for="scheme-phone">Phone Number</label>
          <input type="tel" id="scheme-phone" required>
        </div>
        
        <div class="form-group">
          <label for="scheme-email">Email</label>
          <input type="email" id="scheme-email" required>
        </div>
        
        <div class="form-group">
          <label for="scheme-address">Full Address</label>
          <textarea id="scheme-address" rows="3" required></textarea>
        </div>
        
        <div class="form-group">
          <label for="scheme-id">ID Proof (Aadhaar/PAN/Voter ID)</label>
          <input type="text" id="scheme-id" required>
        </div>
        
        <div class="form-group">
          <label for="scheme-income">Annual Income (₹)</label>
          <input type="number" id="scheme-income" min="0" required>
        </div>
        
        <div class="form-group">
          <label for="scheme-family">Family Members</label>
          <input type="number" id="scheme-family" min="1" required>
        </div>
        
        <div class="form-group">
          <label for="scheme-docs">Documents Uploaded (list)</label>
          <textarea id="scheme-docs" rows="2" required></textarea>
        </div>
  ...
  "Education": (feed) => `
    <div class="nysa-form-container">
      <h3>Enquire About ${feed.details.title}</h3>
      <form id="educationForm">
        ...
        <div class="form-group">
          ...
          <input type="number" id="student-age" min="3" max="30" required>
        </div>
        
        <div class="form-group">
          <label for="student-grade">Current Grade/Class</label>
        ...
        <div class="form-group">
          <label for="interest-subjects">Subjects of Interest</label>
          <textarea id="interest-subjects" rows="2" required></textarea>
        </div>
        
        <div class="form-group">
          <label for="preferred-time">Preferred Class Time</label>
          <input type="text" id="preferred-time" placeholder="e.g., Weekdays 4-6pm">
        </div>
        
        <div class="form-group">
          <label for="education-notes">Additional Notes</label>
          <textarea id="education-notes" rows="2"></textarea>
        </div>
...
function renderNysaFeedCards(filteredData) {
  return filteredData.map(feed => `
   ...
   <button class="feed-action-button" onclick="handleFeedAction('${feed.id}', event)" 
...
function handleFeedAction(feedId, event) {
  if (event) event.stopPropagation();
  
  const feed = nysaFeedData.find(item => item.id === feedId);
  if (!feed) return;

  // Check if form is already open for this feed
  const existingForm = document.getElementById(`nysa-form-${feedId}`);
  if (existingForm) {
  ...
  showFormForFeed(feed);
}

function showFormForFeed(feed) {
  // Create form overlay
  const formOverlay = document.createElement('div');
  formOverlay.className = 'nysa-form-overlay';
  formOverlay.id = `nysa-form-${feed.id}`;
  
  // Get the form template for this category
  const formTemplate = nysaFeedForms[feed.category] || nysaFeedForms["Service Providers"];
  
  // Set the overlay content
  formOverlay.innerHTML = `
    <div class="nysa-form-modal">
      <div class="nysa-form-header">
        <button class="close-form-btn" onclick="closeForm('${feed.id}')">
          <i class="fas fa-times"></i>
        </button>
        <h3>${feed.details.actionButton.text}</h3>
      </div>
      ${formTemplate(feed)}
    </div>
  `;
  ...
  // Add form styles if they don't exist
  addFormStyles();
...
function submitForm(feedId) {
  const feed = nysaFeedData.find(item => item.id === feedId);
  if (!feed) return;
  
  const formOverlay = document.getElementById(`nysa-form-${feedId}`);
  ...
  let message = `*New ${feed.category} Request*\n\n`;
  message += `*Service/Item:* ${feed.details.title}\n`;
  message += `*Provider:* ${feed.details.provider}\n\n`;
  
  // Add form data to message
  for (const [key, value] of Object.entries(formData)) {
    if (value) {
      const label = form.querySelector(`label[for="${key}"]`)?.textContent || key;
      message += `*${label.trim().replace(':', '')}:* ${value}\n`;
    }
  }
  
  // Send via WhatsApp if available
  if (feed.primarySupport?.whatsapp) {
    const whatsappUrl = `https://wa.me/${feed.primarySupport.whatsapp.replace(/\D/g, '')}?text=${encodeURIComponent(message)}`;
    window.open(whatsappUrl, '_blank');
  } 
  // Fallback to email if WhatsApp not available
  else if (feed.primarySupport?.email) {
    const subject = `New ${feed.category} Request: ${feed.details.title}`;
    const mailtoUrl = `mailto:${feed.primarySupport.email}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(message)}`;
...
function addFormStyles() {
  if (document.getElementById('nysa-form-styles')) return;
  
  const style = document.createElement('style');
  style.id = 'nysa-form-styles';
  style.textContent = `
    .nysa-form-overlay {
      position: fixed;
      top: 0;
...
function renderServices(services, category) {
  ...
  return services.map(service => {
    if (category === "Grocery" || category === "Electronics") {
      ...
      `;
      // In the renderServices function, update the electronics product section:
  ...
  return `
    <div class="product-item" data-name="${service.name}" data-price="${service.price.replace('₹','')}">
      ...
      <div class="product-details">
        ...
        ${renderVariants(service.variants)}
        <div class="product-actions">
          <button class="add-to-cart" onclick="addToCart(event, '${service.name}', ${service.price.replace('₹','').replace(',','')})">
            <i class="fas fa-cart-plus"></i> Add to Cart
          </button>
          <button class="buy-now" onclick="showOrderForm(event, '${service.name}', ${service.price.replace('₹','').replace(',','')})">
            <i class="fas fa-bolt"></i> Buy Now
          </button>
        </div>
      </div>
